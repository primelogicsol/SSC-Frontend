...
import {HandCraftProduct} from "../../types/giftShopProductType"
...
const components: PortableTextComponents = {
    block: {
      ...
      normal: ({ children }) => <p className="list-disc pl-5 text-fixnix-gray space-y-2">{children}</p>,
    },
  
    // ✅ Add this section for inline decorators
    marks: {
        indent: ({ children }) => (
        <span className=" ml-3 list-disc pl-5 text-fixnix-gray space-y-2">{children}</span>
      ),
...
const handCraftProductDetail : React.FC<Props> = ({ data  }) =>  {
...
minDate.setDate(currentDate.getDate() +   (data?.shippingEstimate?.minDays || 0));

const maxDate = new Date(currentDate);
maxDate.setDate(currentDate.getDate() + (data?.shippingEstimate?.maxDays || 0));

// Format dates as string (e.g. "June 22" or "22 June")
const formattedMin = format(minDate, "MMMM d");
const formattedMax = format(maxDate, "MMMM d");

const estimatedRange = `${formattedMin} - ${formattedMax}`;
  ...
  const handleQuantityChange = (value: number) => {
    ...
    setQuantity(newQuantity > data.quantity ? data.quantity : newQuantity);
  };
  ...
  const stockStatusText = data.quantity > 10 
    ? "In Stock" 
    : product.stock > 0 
  ...
  // Stock status color
  const stockStatusColor = data.quantity > 10 
    ? "text-green-600" 
    : data.quantity > 0 
    ? "text-orange-500" 
    : "text-red-600";
    ...
    <Layout headerStyle={2} footerStyle={1} breadcrumbTitle="Product Details">
      ...
      <div ref={topRef} className="container mx-auto px-4 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              ...
              >
                ...
                {product.discount > 0 && (
                  <span className="absolute top-4 left-4 bg-fixnix-lightpurple text-white px-2 py-1 text-sm font-bold rounded">
                    -{(data.price / (data.discount || 0)) * 100}% OFF
                  </span>
                )}
              </div>
            </div>
          ...
          <div>
            <h1 className="text-3xl font-bold text-fixnix-darkpurple mb-2">{data.title}</h1>
            <p className="text-fixnix-gray mb-4">{data.subTitle}</p>
            ...
            <div className="mb-6 flex items-center">
              ...
              <span className="ml-2 font-medium">{estimatedRange}</span>
            </div>
          ...
          <div className="bg-white p-6 shadow-sm rounded-lg">
            ...
            {activeTab === "description" && (
              <div className="prose max-w-none">
                <h2 className="text-2xl font-bold text-fixnix-darkpurple mb-4">{data.description?.mainHeading}</h2>
                
                <div>
                    {data.description?.descriptions?.map((item , idx)=>{
                        return (
                            <div>
                                <h3 className="text-xl font-bold text-fixnix-darkpurple mb-3">{item.heading}</h3>
            ...
            {activeTab === "shipping" && (
              <div>
                <h2 className="text-2xl font-bold text-fixnix-darkpurple mb-4">{data.shipping?.mainHeading}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {data.shipping?.details?.map((item , idx)=>{
                        return (
                            <div key={idx} >
                    <h3 className="text-xl font-bold text-fixnix-darkpurple mb-3">{item.heading}</h3>
                    <div className="text-fixnix-gray mb-4">
...
export default handCraftProductDetail